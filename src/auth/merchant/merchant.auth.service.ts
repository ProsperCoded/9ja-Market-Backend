...
export class MerchantAuthService implements IAuthService {
  ...
  initializeEventHandlers() {
    ...
    this.eventEmiter.on(
      `sendMerchantEmailVerificationEmail`,
      async (data: {
        email: string;
        token: string;
    ...
    this.eventEmiter.on(
      ...
      async (data: { email: string; brandName: string }) => {
  ...
  async login(loginData: LoginRequestDto): Promise<LoginResponseDto> {
    try {
      ...
      const refreshToken = this.getToken(
        { email: merchant.email, refreshToken: _refreshToken },
        "7d"
      );
  ...
  async register(
    registerData: MerchantRegisterRequestDto,
    url: string
  ): Promise<boolean> {
    ...
    const merchantBrand =
      await this.merchantRepository.getMerchantByBrandName(brandName);
    if (merchantBrand) {
    ...
    try {
      ...
      await this.merchantRepository.update(newMerchant.id, {
        emailVerificationCode: verificationCode,
      });
      const _token = this.getToken({ email, verificationCode });
      const token = encodeURIComponent(_token);
      this.eventEmiter.emit("sendMerchantEmailVerificationEmail", {
        email,
        token,
        verificationCode,
        url,
  ...
  async forgotPassword(
    ...
    this.eventEmiter.emit("sendMerchantPasswordResetEmail", {
      email,
      resetCode,
    });
    return true;
  ...
  async googleCreateOrLogin(profile: any): Promise<string> {
    ...
    try {
      ...
      if (!merchant)
        existingEmail =
          await this.merchantRepository.getMerchantByEmail(emailValue);
      if (!existingEmail && !merchant) {
        ...
        const _refreshToken = cryptoService.random();
        const refreshToken = this.getToken(
          { email: newMerchant.email, refreshToken: _refreshToken },
          "7d"
        );
        await this.merchantRepository.update(newMerchant.id, {
          refreshToken: _refreshToken,
        ...
        const result = this.getToken(
          { id: newMerchant.id, accessToken, refreshToken },
          "7m"
        );
        ...
        const _refreshToken = cryptoService.random();
        const refreshToken = this.getToken(
          { email: merchantData.email, refreshToken: _refreshToken },
          "7d"
        );
        ...
        const result = this.getToken(
          { id: merchantData.id, accessToken, refreshToken },
          "7m"
        );
        return encodeURIComponent(result);
      }
